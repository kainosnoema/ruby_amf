...
VALUE rb_read(load_context_t* context, uint8_t type);
static VALUE t_deserialize(VALUE self, VALUE string);

VALUE rb_read_number(load_context_t* context)
{
...
VALUE rb_read_string(load_context_t* context, bool is_long)
{
  uint32_t len = 0;
...
VALUE rb_read_object(load_context_t* context, bool add_to_ref_cache)
{
  ...
  while (1) {
    ...
    rb_hash_aset(object, key, rb_read(context, type));
  }
...
VALUE rb_read_array(load_context_t* context)
{
  ...
  for (i=0;i<len;i++) {
    uint8_t type = c_read_word8(context);
    rb_ary_push(object, rb_read(context, type));
  }
...
VALUE rb_read_hash(load_context_t* context)
{
  ...
  if (rb_str_cmp( rb_big2str(rb_str_to_inum(key, 10, Qfalse),10),  key) == 0) {
    ...
    rb_ary_store(object, NUM2LONG(rb_str_to_inum(key, 10, Qfalse)), rb_read(context, type));
    while (1) {
      VALUE key = rb_str_to_inum(rb_read_string(context, 0), 10, Qfalse);
      uint8_t type = c_read_word8(context);
      if (type == AMF0_OBJECT_END) { break; }
      rb_ary_store(object, NUM2LONG(key), rb_read(context, type));
    }
...
VALUE rb_read_date(load_context_t* context)
{
...
VALUE rb_read_xml(load_context_t* context)
{
  return rb_read_string(context, 1);
}

VALUE rb_read_typed_object(load_context_t* context)
{
...
{  
  switch (type) {
    ...
    case AMF0_NUMBER:       return rb_read_number(context);
    case AMF0_BOOLEAN:      return rb_read_boolean(context);
    case AMF0_STRING:       return rb_read_string(context, 0);
    case AMF0_OBJECT:       return rb_read_object(context, 1);
    ...
    case AMF0_REFERENCE:    return rb_read_reference(context);
    case AMF0_HASH:         return rb_read_hash(context);
    case AMF0_STRICT_ARRAY: return rb_read_array(context);
    case AMF0_DATE:         return rb_read_date(context);
    case AMF0_LONG_STRING:  return rb_read_string(context, 1);
    ...
    case AMF0_XML:          return rb_read_xml(context);
    case AMF0_TYPED_OBJECT: return rb_read_typed_object(context);
  }
...
static VALUE t_deserialize(VALUE self, VALUE string)
{
  ...
  uint8_t type = c_read_word8(context);
  return rb_read(context, type);
}

void Init_ruby_amf_AMF0Deserializer() {
